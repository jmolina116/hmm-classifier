...
def compute_cm(classifier, test_data):
    ...
    Returns:
        Confusion matrix
    """
    confusion_matrix = ConfusionMatrix(classifier.model['label2index'],
                                       classifier.model['index2label'])
...
class ConfusionMatrix(object):
    ...
    def print_out(self):
        ...
        for i in range(self.num_classes):
            row = [self.index2label[i]] + \
                  [str(self.matrix[i, j]) for j in range(len(self.matrix[i,]))]
            rows.append(row)
        print('\n\nConfusion Matrix')
        print('--------------')
        ...
        for i in range(self.num_classes):
            print('{} \tprecision {} \trecall {}\t F1 {}'.format(
                  self.index2label[i], precision[i], recall[i], f1[i]))
